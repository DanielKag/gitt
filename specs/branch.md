...
| BR-11  | When showing (SUM-13), the bordered "ai summary" panel sits below the list and reflects the **selected** branch. Selecting a branch auto-loads its summary from the on-disk cache (keyed by the branch tip SHA, prefixed so it never collides with a commit summary); a hit shows instantly (and reveals the panel), a miss shows the "press @" hint once the panel is open. | unit, e2e  |
